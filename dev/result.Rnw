...
def signal_octet_to_latex(df, signals):
    """Generate a table for 8 signals: 4 pairs stacked vertically."""
    ...
    # First pair header with column abbreviations
    header1 = r' & \multicolumn{6}{c|}{' + sigs_esc[0] + r'} & \multicolumn{6}{c}{' + sigs_esc[1] + r'} \\'
    lines.append(header1)
    header2 = 'Stat & ' + ' & '.join(abbrevs) + ' & ' + ' & '.join(abbrevs) + r' \\'
    lines.append(header2)
    lines.append(r'\midrule')

    # First pair data
    for i in range(len(dfs[0])):
        stat_name = dfs[0].iloc[i]['name'].replace('_', r'\_')
        cells = [stat_name]
        for col in cols:
            cells.append(format_val(dfs[0].iloc[i][col]))
        for col in cols:
            cells.append(format_val(dfs[1].iloc[i][col]))
        lines.append(' & '.join(cells) + r' \\')

    # Remaining pairs (no column abbreviations, just signal names)
    for p in range(1, len(signals) // 2):
        idx1, idx2 = p * 2, p * 2 + 1
        if idx1 >= len(signals):
            break
        lines.append(r'\midrule')
        header = r' & \multicolumn{6}{c|}{' + sigs_esc[idx1] + r'} & \multicolumn{6}{c}{' + sigs_esc[idx2] + r'} \\'
        lines.append(header)
        lines.append(r'\midrule')

        for i in range(len(dfs[idx1])):
            stat_name = dfs[idx1].iloc[i]['name'].replace('_', r'\_')
            cells = [stat_name]
            for col in cols:
                cells.append(format_val(dfs[idx1].iloc[i][col]))
            for col in cols:
                cells.append(format_val(dfs[idx2].iloc[i][col]))
            lines.append(' & '.join(cells) + r' \\')
