...
def init_worker(amt, prices, chain, memoize, prices_dict):
    ...
    # No need to warm up DuckDB connection if using prices_dict
...
def main():
    parser = argparse.ArgumentParser(
        description="Run straddle valuations for all straddles matching a pattern"
    )
    ...
    # Count straddles per (asset, year, month)
    ...
    # Build task list (without paths - workers use globals)
    ...
    # Preload all prices into memory for fast lookups
    # Entry can be 1 month before expiry, so start from (start_year-1)
    ...
    # Sort rows by asset, straddle, date for consistent output
    if columns and all_rows:
        # Find indices for sorting
